    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($foto)) {
            if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
                ...
                $sql = "insert into petugas (kode_petugas,nama_petugas,nip,jk,email,no_telp,alamat,foto) values
                    ('$kode_petugas','$nama','$nip','$jk','$email','$no_telp','$alamat','$foto')";
            }
        } else {
        ...
        if ($simpan_petugas and $simpan_pengguna) {
            ...
            header("Location:../../dist/index.php?page=petugas&add=gagal");
        }
    }
}
?>
...
$tahun = date('y');
$kode_depan = $tahun . '01';
$nip_petugas = $kode_depan . sprintf("%03s", $id_petugas);
...
<form action="petugas/tambah.php" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="text" name="nip" class="form-control" value="<?php echo isset($nip) ? $nip : ''; ?>" placeholder="Masukkan Nomor Induk Pegawai" required>
    </div>
</div>
    </div>
    <div class="row">
...
<!-- JavaScript untuk tampilan preview gambar -->
<script>
